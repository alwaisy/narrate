...
### Testing
• Run pipeline test: `bash main.sh scout` (validates the entire pipeline)
...
```
├─ .gemini/commands/ → TOML command definitions for Gemini CLI (go, review, reddit, article, product, text, reset, init, comment, poster)
├─ .commands/ → Kilo CLI command definitions
├─ context/ → Strategic mandates (ICP, content pillars, identity rules, master execution rules, comment rules, poster rules)
├─ templates/ → LinkedIn post templates JSON, anti-AI rules, template metadata index
├─ lib/ → Bash scripts for data acquisition (reddit, fetch_thread, fetch_article, fetch_product, build_index, get_template, generate_comments)
...
The `.gemini/commands/` directory holds 10 TOML command definitions that drive the AI layer. Each TOML contains a `description` and a `prompt` field. `go.toml` is the master orchestrator: it runs `bash main.sh scout`, selects 3 threads, loads all context files, and executes the 7-step protocol. `review.toml` handles post-quality audits. `text.toml` and `reddit.toml` handle single-source transformations. `article.toml` and `product.toml` fetch external content via `fetch_article.sh` or `fetch_product.sh` before drafting. `reset.toml` clears pipeline state. `init.toml` scans the codebase to create or update AGENTS.md. `comment.toml` and `poster.toml` handle comment generation and poster data blocks.
...
### Coding Style
• Language: Bash scripts for pipeline, JavaScript (ESM) for dashboard, Markdown for prompts and context
• Bash conventions: UPPER_CASE for globals, `set -euo pipefail` in main.sh, `source config.sh` pattern in all lib scripts
• JavaScript: Hono framework with pino structured logging, camelCase variables, PascalCase for classes, 2-space indentation
...
• JavaScript: Use ESM imports with `import` syntax; CommonJS-style requires where possible
